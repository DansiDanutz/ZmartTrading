...
Tests the entire password change flow including login, code requests, and password updates
"""
...
def main():
    print("🚀 COMPLETE PASSWORD CHANGE TEST")
    print("Testing the entire password change flow")
    ...
    print_step(3, "REQUEST PASSWORD CHANGE CODE")
    code_data = {
        "email": LOGIN_EMAIL
    }
    
    response = make_request('POST', '/api/test/request-password-change', code_data, session_cookies)
    if not response or response.status_code != 200:
        print("❌ Password change code request failed")
        return False
    
    print("✅ Password change code requested")
    
    # Step 4: Get the verification code (using test endpoint)
    print_step(4, "GET VERIFICATION CODE")
    ...
    print_step(5, "CHANGE PASSWORD")
    change_data = {
        "code": verification_code,
        "new_password": NEW_PASSWORD
    }
    
    response = make_request('POST', '/api/test/change-password', change_data, session_cookies)
    if not response or response.status_code != 200:
    ...
    print("✅ Complete password change flow works correctly")
    print(f"{'='*60}")
